...
public class AuthorSearchPanelTest extends JFrame {
	...
	public void authorSearchPanelViewTest() {
		...
		JButton goToFavorites = asp.getGotoFavorires();
		goToFavorites.doClick();
		JButton paperSearch = asp.getPaperSearchButton();
		paperSearch.doClick();
		JButton titleSearch = asp.getTitleSearchButton(searchPane);
		titleSearch.doClick();
		JButton search = asp.getSearchButton();
		search.doClick();
		JButton cancel = asp.getCancelButton();
		cancel.doClick();
		assertEquals("First Name:", asp.getFirstNameLabel().getText());
